...
func ExampleGroup_parallel() {
	Google := func(ctx context.Context, query string) ([]Result, error) {
		g, ctx := berrgroup.WithContext(ctx)
...
func TestZeroGroup(t *testing.T) {
	err1 := errors.New("berrgroup_test: 1")
	err2 := errors.New("berrgroup_test: 2")
...
func TestWithContext(t *testing.T) {
	...
	for _, tc := range cases {
		g, ctx := berrgroup.WithContext(context.Background())
...
func TestGoPanicRecover(t *testing.T) {
	g, ctx := berrgroup.WithContext(btrace.SetTraceID(bcontext.New()))
	g.Go(func() error {
